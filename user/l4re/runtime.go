...
//  3. Install a signal handler with l4re_itas for the one operation the
//     Go runtime performs during startup under the assumption of running
//     either on bare metal or on Linux (amd64: FS_BASE installation via
//     arch_prctl; arm64: MRS of privileged ID registers). The handler
//     completes or emulates the operation on the faulting thread, see
//     sig_$GOARCH.s. It is removed again once the World is started.
//  4. Publish the region as runtime/goos.RamStart/RamSize, place the
...
func earlyInit() {
	...
	if err := svc.Sigaction(bootSignal, bootHandler(), svc.SaSiginfo); err != 0 {
		earlyFatal("l4re: sigaction failed: ", err)
	}
}
...
// hwinit1 runs after the World is started. The startup signal handler has
// served its purpose; from here on faults get itas' default treatment.
//
//go:linkname hwinit1 runtime/goos.Hwinit1
func hwinit1() {
	svc.Sigaction(bootSignal, svc.SigDfl, 0)
	goos.Exit = exit
}
